...
''', elem_id='connector-plus-html')
        ...
        with gr.Row(elem_id='chat-controls', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                ...
                with gr.Accordion('Custom AI style', open=False):
                ...
                with gr.Accordion('Google Workspace Copilot', open=False):
                    gr.Markdown('Quick links: [Google Docs API](https://developers.google.com/docs/api/quickstart/python) · [Google Slides API](https://developers.google.com/slides/api/quickstart/python) · [Google Drive API](https://developers.google.com/drive/api/quickstart/python) · [GitHub Tokens](https://github.com/settings/tokens) · [Google Classroom API](https://developers.google.com/classroom)')
                    gr.Markdown('How to connect: 1) Create a Google Cloud **service account**. 2) Enable Docs/Slides/Drive APIs. 3) Download JSON key file. 4) Paste full path below. 5) Share your target Doc/Slides with the service-account email (`...@...iam.gserviceaccount.com`).')
                    gr.Markdown('Your JSON key should include fields like: `type`, `project_id`, `private_key_id`, `private_key`, `client_email`, `token_uri`.')
                    ...
                    gr.Markdown('Tip: full student learning UI is in **Lessons** tab (left sidebar, below Session).')
